...
def alpaca_account():
    ...
    if not key or not secret:
        missing = []
        if not key: missing.append("ALPACA_API_KEY")
        if not secret: missing.append("ALPACA_SECRET_KEY")
        return None, "Streamlit Secrets missing: " + ", ".join(missing)
    try:
...
def indicators(df, end=None):
    ...
    av = d["volume"].rolling(20).mean().iloc[-1]
...
def submit_paper_order(symbol, side, qty):
    ...
    if not alpaca_headers(): return False,"Alpaca PAPER credentials are unavailable."
    try:
        payload={"symbol":symbol,"qty":str(int(qty)),"side":side,"type":"market","time_in_force":"day"}
        r=requests.post(f"{ALPACA_TRADE_URL}/v2/orders",headers={**alpaca_headers(),"Content-Type":"application/json"},json=payload,timeout=15)
...
if account:
    ...
    st.caption("The app is using ALPACA_API_KEY and ALPACA_SECRET_KEY from Streamlit Secrets. Secret values are never displayed.")

c1,c2,c3,c4=st.columns(4)
c1.metric("System","ONLINE")
c2.metric("Broker","ALPACA • PAPER" if account else "ALPACA • ERROR")
...
with st.sidebar:
    ...
    max_risk=st.slider("Max risk / trade (%)",.1,2.0,1.0,.1)
    max_vol=st.slider("Max volatility (%)",20.0,100.0,55.0,1.0)
    st.divider(); st.header("🧪 Backtest Settings")
    period=st.selectbox("Historical test period",["6mo","1y","2y","3y","5y"],index=1)
    capital=st.number_input("Starting paper capital ($)",100.0,1000000.0,1000.0,100.0)
    allocation=st.slider("Capital allocated per trade (%)",5.0,100.0,50.0,5.0)
    hold_days=st.number_input("Automatic maximum hold (trading days)",1,252,6,1)

stocks=list(dict.fromkeys([s.strip().upper() for s in stock_text.replace(",","\n").splitlines() if s.strip()]))[:10] or ["AAPL"]
st.header("📊 Alpaca Market Scan")
rows=[]; results={}
for ticker in stocks:
    hist,err=load_history(ticker,"6mo")
    if hist is None:
        rows.append({"Ticker":ticker,"MAXPROFIT":"DATA ERROR","Score":0,"AI":"BLOCK","Confidence":0,"Risk":"BLOCK"})
        continue
    ind=indicators(hist); algo=maxprofit(ind); risk=risk_gate(ind,algo,max_risk,max_vol); ai=ai_helper(ind,algo,risk)
    results[ticker]=(ind,algo,risk,ai)
...
st.dataframe(pd.DataFrame(rows).sort_values(["Score","Confidence"],ascending=False),use_container_width=True,hide_index=True)

if results:
    selected=st.selectbox("Select stock",list(results))
    ind,algo,risk,ai=results[selected]
    ...
    st.caption("These buttons can only send PAPER orders. Live trading is hard-disabled.")
...
st.caption("Walk-forward simulation. Signals use only data available at each point; entries/exits use the following trading day's open. Automatic maximum hold defaults to 6 trading days.")
if st.button("▶️ RUN SIMULATION",type="primary",use_container_width=True):
    if not alpaca_headers():
        st.error(account_error or "Alpaca PAPER credentials are unavailable.")
    else:
        histories={}; errors=[]
